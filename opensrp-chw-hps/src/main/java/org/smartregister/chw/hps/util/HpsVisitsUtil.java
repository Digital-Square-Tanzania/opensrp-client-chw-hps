...
import org.smartregister.chw.hps.repository.VisitDetailsRepository;
...
public class HpsVisitsUtil extends VisitUtils {
    ...
    public static void processVisits() throws Exception {
        processVisits(HpsLibrary.getInstance().visitRepository(), HpsLibrary.getInstance().visitDetailsRepository());
    }
    ...
    public static String getHpsVisitStatus(Visit lastVisit) {
    ...
    public static String getHpsServiceVisitStatus(Visit lastVisit) {
        HashMap<String, Boolean> completionObject = new HashMap<>();
        try {
            JSONObject jsonObject = new JSONObject(lastVisit.getJson());
            JSONArray obs = jsonObject.getJSONArray("obs");
    ...
    public static void manualProcessVisit(Visit visit) throws Exception {
        ...
        VisitDetailsRepository visitDetailsRepository = HpsLibrary.getInstance().visitDetailsRepository();
        VisitRepository visitRepository = HpsLibrary.getInstance().visitRepository();
